...
//create additional assistant ui

function awaitMessage(server) {
...
var standard_color = document.documentElement.style.getPropertyValue('--primary');
async function communicateToMica(command_dict) {
  try {
      let server = await initWebsocket('ws://localhost:7800',null,1500,0)
      server.send(JSON.stringify(command_dict))
      // ... use server
      const {data} = await awaitMessage(server)
      server.close()
      const result = JSON.parse(data)
      console.log(result)

      if (found_server == false){
         //document.getElementById("assistant-ui").style = ""
         document.getElementById("uiToggle").style=""
         document.getElementById("assistant-input").addEventListener('keyup', function(event) {
            if (event.key === "Enter") {
      ...
      document.documentElement.style.setProperty('--primary', standard_color);
      document.getElementById("assistant-input").placeholder = 'Connection to live assistant active, type "help"';
      found_server = true
      if (result == "Executed Command"){}
      else if (typeof result != 'string' && text_old != result.html_printout) {
         text_old = result.html_printout
         document.getElementById("assistant-output").innerHTML = text_old
         
         document.getElementById("assistant-output").scrollTo(0,document.getElementById("assistant-output").scrollHeight)
         //document.getElementById("assistant-inout").scrollTo(0,0)//scrollIntoView({behavior: "smooth"})
      }
  } catch (error) {
      console.log("ooops ", error)
      if (found_server == true) {
         document.getElementById("assistant-input").placeholder = "Connection to live assistant lost...";
         document.documentElement.style.setProperty('--primary', "#f8431f");
         return
      }
  }
}
...
async function updateAssistant() {
   if (active_flag == false){
      ...
      await communicateToMica({"command":"get_dict"})
      active_flag = false
   } else {
      console.log("waiting for previous update")
   }
}

async function sendCommand(){
   await communicateToMica({"command":"interpret_command","value":document.getElementById("assistant-input").value})
...
window.addEventListener('load', () => {
   ...
   setInterval(updateAssistant, 2000);
});